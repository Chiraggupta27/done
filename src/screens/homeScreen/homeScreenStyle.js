const image ={height:80,width:260, alignSelf:"center" , position:"relative" , top:50}
const image1 ={height:30 , width:30 , position:"absolute" ,left:50}
const text1 ={fontSize:25, fontWeight:"bold" , alignSelf:"center",position:"relative" ,top:100}
const text2 ={fontSize:17,color:"grey" , position:"relative" , top:110,alignSelf:"center"}
const text3 ={fontSize:15,color:"grey",position:"relative", top:150 , left:30}
const text4 ={fontSize:15,color:"grey",position:"relative", top:180 , left:30}
const text5 ={fontSize:15 , color:"grey" ,position:"relative", top:210, left :275 }
const text6 ={textAlign:"center",padding:13, color:"white"}
const text7 ={color:"grey" , textAlign:"center" ,position:"absolute" ,top:310 ,left:80 }
const text8 ={color:"orange" , textAlign:"center" ,position:"absolute" ,top:310 ,left:240 }
const text9 = {alignSelf:"center" , color:"white"}
const text10 = {alignSelf:"center"}
const textInput ={height:50,width:370, borderRadius:10 , position:"relative" , top:165 , left:30  ,padding:10, fontSize:20 ,backgroundColor:"lightgrey"}
const textInput1 ={color:"grey",height:50,width:370,borderRadius:10 , position:"relative" , top:195 , left:30 ,padding:10,fontSize:20 ,backgroundColor:"lightgrey"}
const touchopacity = {alignSelf:"center",position:"absolute",top:250,height:50,width:330 , left:50 , borderRadius:10 , backgroundColor:"orange"}
const touchopacity1 = {alignSelf:"center",position:"absolute",top:480,height:50,width:330 , left:50 , borderRadius:10 , backgroundColor:"blue" , justifyContent:"center"}
const touchopacity2 = {alignSelf:"center",position:"absolute",top:550,height:50,width:330 , left:50 , borderRadius:10 , backgroundColor:"lightgrey" ,justifyContent:"center"}


export default {
    image,
    text1,
    text2,
    text3,
    text4,
    textInput,
    textInput1,
    text5,
    touchopacity,
    text6,
    text7,
    text8,
    touchopacity1,
    touchopacity2,
    text9,
    text10,
    image1,
}